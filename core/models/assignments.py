...
class Assignment(db.Model):
    ...
    @classmethod
    def gradeAssignment(cls, _id, gradeOfAssignment, principal: Principal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(assignment.teacher_id == principal.teacher_id, 'This assignment belongs to some other teacher')
        assertions.assert_valid(gradeOfAssignment in ['A', 'B', 'C', 'D'], 'assignment with empty content cannot be submitted')

        assignment.grade = gradeOfAssignment
